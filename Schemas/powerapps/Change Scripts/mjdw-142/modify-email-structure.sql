...
DROP FUNCTION IF EXISTS powerapps.sp_sagitta_account_email_upsert(int);
GO 
...
BEGIN 
	...
	WHERE 	(email.email,email.modify_dt) 
		IS DISTINCT FROM 	(excluded.email,excluded.modify_dt);
