...
BEGIN 
	...
	SELECT 	 uuid_generate_v4() 												AS suspect 
			...
			,'Employee Benefits' 												AS line_of_business 
			...
			,CASE 
				WHEN benefits_renewal IS NOT NULL THEN 
					CASE
						WHEN concat_ws('-', date_part('year', current_date)::TEXT, LEFT(LEFT(eb.benefits_renewal, 5),2), right(LEFT(eb.benefits_renewal, 5),2))::date >= current_date
							THEN concat_ws('-', date_part('year', current_date)::TEXT, LEFT(LEFT(eb.benefits_renewal, 5),2), right(LEFT(eb.benefits_renewal, 5),2))::date
						ELSE concat_ws('-', date_part('year', current_date + INTERVAL '1 year')::TEXT, LEFT(LEFT(eb.benefits_renewal, 5),2), right(LEFT(eb.benefits_renewal, 5),2))::date
					END
				ELSE NULL 
			 END 																AS benefits_renewal
			,eb.benefits_broker_normalized 										AS benefits_broker 
			,REPLACE(eb.broker_share, '%', '')::double PRECISION 				AS broker_share
			,eb.broker_affiliation 												AS broker_affiliation 
			,eb.broker_revenue::int 											AS broker_revenue 
			,replace(eb.broker_revenue_pct, '%', '')::double PRECISION 			AS broker_revenue_pct 
			,eb.broker_commission::int 											AS broker_commission 
			,replace(eb.broker_commission_pct, '%', '')::double PRECISION 		AS broker_commission_pct 
			,eb.broker_fees::int 												AS broker_fees 
			,replace(eb.broker_fees_pct, '%', '')::double PRECISION 			AS broker_fees_pct 
			,eb.largest_carrier_normalized 										AS largest_carrier 
			,eb.carrier_premium::int 											AS carrier_premium 
			,eb.tpa 															AS tpa 
			,eb.tpa_compensation 												AS tpa_compensation 
			,eb.advisor 														AS advisor 
			,eb.service_provider 												AS service_provider 
			,eb.plan_assets::bigint 											AS plan_assets 
			,eb.no_of_plans 													AS number_of_plans 
			,eb.corrective_distributions::int 									AS corrective_distributions 
			,eb.participant_loans::int 											AS participant_loans 
			,eb.employees 														AS number_of_employees 
			,eb.eligible_employees 												AS eligible_employees 
			,eb.retirees 														AS retirees 
			,eb.premium::int 													AS premium 
			,REPLACE(eb.premium_per_employee,',','')::double PRECISION 			AS premium_per_employee 
			,eb.revenue::int 													AS revenue 
			,eb.commission::int 												AS commission 
			,eb.fees::int 														AS fees 
