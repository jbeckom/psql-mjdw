DROP FUNCTION IF EXISTS powerapps.sp_suspect_miedge_eb_upsert();
GO 
...
BEGIN 
	...
	SELECT 	 public.uuid_generate_v4() 					AS suspect 
			...
			,CASE 
				WHEN benefits_renewal IS NOT NULL THEN 
					CASE
						WHEN concat_ws('-', date_part('year', current_date)::TEXT, LEFT(LEFT(benefits_renewal, 5),2), right(LEFT(benefits_renewal, 5),2))::date >= current_date
							THEN concat_ws('-', date_part('year', current_date)::TEXT, LEFT(LEFT(benefits_renewal, 5),2), right(LEFT(benefits_renewal, 5),2))::date
						ELSE concat_ws('-', date_part('year', current_date + INTERVAL '1 year')::TEXT, LEFT(LEFT(benefits_renewal, 5),2), right(LEFT(benefits_renewal, 5),2))::date
					END
				ELSE NULL 
