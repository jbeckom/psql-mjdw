DROP FUNCTION IF EXISTS powerapps.sp_suspect_miedge_pc_upsert();
GO 
...
BEGIN 
	...
	SELECT 	 public.uuid_generate_v4() 									AS suspect
			...
			,pc.ex_mod 													AS ex_mod 
			,pc.est_annual_prem 										AS estimated_annual_premium
			...
			,CASE 
				WHEN pc.dot IS NOT NULL THEN 
					concat_ws('-', RIGHT(pc.dot, 4), LEFT(pc.dot,2), '01')::date
				ELSE NULL 
			 END 														AS dot_date 
			,CASE 
				WHEN pc.osha IS NOT NULL THEN 
					concat_ws('-', RIGHT(pc.osha, 4), LEFT(pc.osha,2), '01')::date
				ELSE NULL 
			 END 														AS osha_date 
			,CASE 
				WHEN pc.whd IS NOT NULL THEN 
					concat_ws('-', RIGHT(pc.whd, 4), LEFT(pc.whd,2), '01')::date
				ELSE NULL 
			 END 														AS whd_date 
			,pc.ein  													AS ein 
