CREATE OR REPLACE VIEW p_and_c.normal_demo
AS SELECT DISTINCT concat('DEMO', '-', a.claim)::character varying(256) AS claim_number,
    ...
    a.expense_reserve AS reserve_expense,
        CASE
            WHEN a.indemnity_paid::text = ''::text THEN '0'::character varying
            ELSE a.indemnity_paid
        END::character varying(256) AS paid_indemnity,
        CASE
            WHEN a.medical_paid::text = ''::text THEN '0'::character varying
            ELSE a.medical_paid
        END::character varying(256) AS paid_medical,
        CASE
            WHEN a.indemnity_reserve::text = ''::text THEN '0'::character varying
            ELSE a.indemnity_reserve
        END::character varying(256) AS reserve_indemnity,
        CASE
            WHEN a.medical_reserve::text = ''::text THEN '0'::character varying
            ELSE a.medical_reserve
        END::character varying(256) AS reserve_medical,
        CASE
            WHEN a.subrogation::text = ''::text THEN '0'::character varying
            ELSE a.subrogation
        END::character varying(256) AS subrogation,
    ((
        CASE
            WHEN a.recovery::text = ''::text THEN '0'::numeric
            ELSE a.recovery::numeric(20,2)
        END +
        CASE
            WHEN a.salvage::text = ''::text THEN '0'::numeric
            ELSE a.salvage::numeric(20,2)
    ...
    a.farm AS claim_site,
    a.jurisdiction_state AS benefit_state,
    a.jurisdiction_state AS garage_state,
    a.occupation,
    a.class_code,
    a.name AS claimant_name,
    split_part(a.name::text, ' '::text, 1) AS claimant_first,
    ''::character varying(256) AS claimant_middle,
    split_part(a.name::text, ' '::text, 2) AS claimant_last,
    ...
    concat(a.cause_of_injury, ' - ', a.cause_group)::character varying(256) AS carrier_cause,
    concat('DEMO', ' - ', a.line_of_business, ' - ', a.type_of_loss)::character varying(256) AS carrier_coverage_type,
    a.nature_of_injury AS carrier_nature,
    a.type_of_injury AS carrier_injury_code,
    concat('DEMO', ' - ', a.insured_name, ' - ', a.line_of_business, ' - ', a.effective_date::date, ' - ', a.policy)::character varying(256) AS carrier_policy_number,
    'raw_demo'::character varying(256) AS source_table,
    a.uploaded_at
   FROM p_and_c.raw_demo a;
GO

/*** PERMISSIONS ***/
ALTER VIEW p_and_c.normal_demo OWNER TO mj_admin;
GO

Grant select on table p_and_c.normal_demo to rl_pc_data_analyst;
GO

Grant select on table p_and_c.normal_demo to rl_pc_rpa_developer;
GO

Grant select on table p_and_c.normal_demo to rpauser;
GO
GRANT SELECT ON p_and_c.normal_demo to rl_p_and_c_r;
GO

GRANT INSERT ON p_and_c.normal_demo to rl_p_and_c_a;
GO

GRANT UPDATE ON p_and_c.normal_demo to rl_p_and_c_w;
GO

GRANT DELETE ON p_and_c.normal_demo to rl_p_and_c_d;
GO