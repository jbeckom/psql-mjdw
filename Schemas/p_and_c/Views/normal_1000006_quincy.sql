CREATE OR REPLACE VIEW p_and_c.normal_1000006_quincy
AS SELECT DISTINCT concat('1000006_quincy', '-', a."Claim Number")::character varying(256) AS claim_number,
    ...
    a."Hierarchy Location Level 4" AS claim_city,
    c.code::character varying(256) AS claim_state,
    ''::character varying(256) AS claim_zip,
    concat(a."Hierarchy Location Level 3", ' - ', a."Hierarchy Location Level 4")::character varying(256) AS claim_site,
    ...
    concat('1000006_quincy', ' - ', a."Coverage Minor")::character varying(256) AS carrier_coverage_type,
    a."Injury" AS carrier_nature,
    split_part(a."Coverage Minor"::text, ' - '::text, 2)::character varying(256) AS carrier_injury_code,
    concat('1000006_quincy', ' - ', a."Hierarchy Location Level 2", ' - ', a."Coverage Major", ' - ', a."Policy Year")::character varying(256) AS carrier_policy_number,
    'raw_1000006_quincy'::character varying(256) AS source_table,
    a.uploaded_at::character varying(256) AS uploaded_at
   FROM p_and_c.raw_1000006_quincy a
     LEFT JOIN p_and_c.states c ON a."Jurisdiction"::text = c.name;
GO

/*** PERMISSIONS ***/
ALTER VIEW p_and_c.normal_1000006_quincy OWNER TO mj_admin;
GO

Grant select on table p_and_c.normal_1000006_quincy to rl_pc_data_analyst;
GO

Grant select on table p_and_c.normal_1000006_quincy to rl_pc_rpa_developer;
GO

Grant select on table p_and_c.normal_1000006_quincy to rpauser;
GO
GRANT SELECT ON p_and_c.normal_1000006_quincy to rl_p_and_c_r;
GO

GRANT INSERT ON p_and_c.normal_1000006_quincy to rl_p_and_c_a;
GO

GRANT UPDATE ON p_and_c.normal_1000006_quincy to rl_p_and_c_w;
GO

GRANT DELETE ON p_and_c.normal_1000006_quincy to rl_p_and_c_d;
GO