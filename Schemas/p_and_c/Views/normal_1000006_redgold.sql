CREATE OR REPLACE VIEW p_and_c.normal_1000006_redgold
AS SELECT DISTINCT concat('1000006_redgold', '-', a."Claim Number")::character varying(256) AS claim_number,
    ...
    a."Hierarchy Location Level 3" AS claim_site,
    ...
    concat('1000006_redgold', ' - ', a."Coverage Minor")::character varying(256) AS carrier_coverage_type,
    a."Injury" AS carrier_nature,
    split_part(a."Coverage Minor"::text, ' - '::text, 2)::character varying(256) AS carrier_injury_code,
    concat('1000006_redgold', ' - ', a."Hierarchy Location Level 2", ' - ', a."Coverage Major", ' - ', a."Policy Year")::character varying(256) AS carrier_policy_number,
    'raw_1000006_redgold'::character varying(256) AS source_table,
    a.uploaded_at::character varying(256) AS uploaded_at
   FROM p_and_c.raw_1000006_redgold a
     LEFT JOIN p_and_c.states c ON a."Jurisdiction"::text = c.name;
GO

/*** PERMISSIONS ***/
ALTER VIEW p_and_c.normal_1000006_redgold OWNER TO mj_admin;
GO

Grant select on table p_and_c.normal_1000006_redgold to rl_pc_data_analyst;
GO

Grant select on table p_and_c.normal_1000006_redgold to rl_pc_rpa_developer;
GO

Grant select on table p_and_c.normal_1000006_redgold to rpauser;
GO
GRANT SELECT ON p_and_c.normal_1000006_redgold to rl_p_and_c_r;
GO

GRANT INSERT ON p_and_c.normal_1000006_redgold to rl_p_and_c_a;
GO

GRANT UPDATE ON p_and_c.normal_1000006_redgold to rl_p_and_c_w;
GO

GRANT DELETE ON p_and_c.normal_1000006_redgold to rl_p_and_c_d;
GO