CREATE OR REPLACE VIEW p_and_c.normal_lmi_cau_pd
AS SELECT DISTINCT concat('LMI_CAU_PD', '-', a."Claim Number")::character varying(256) AS claim_number,
    ...
    a."Loss OR" AS reserve_indemnity,
    ...
    ''::character varying(256) AS benefit_state,
        CASE
            WHEN a."Garage State"::text = ''::text THEN a."Accident State"
            WHEN a."Garage State" IS NULL THEN a."Accident State"
            ELSE a."Garage State"
        END AS garage_state,
    ...
    split_part(split_part(a."Insured Driver Name"::text, ','::text, 2), ' '::text, 1) AS driver_first,
    ...
    concat('LMI_CAU_PD', ' - ', 'CAU_PD', ' - ', a."Accident Description Code")::character varying(256) AS carrier_coverage_type,
    ...
    concat('LMI_CAU_PD', ' - ',
        CASE
            ...
            ELSE a."Client"
        END, ' - ', a."Contract Effective Date"::date, ' - ', 'CAU_PD')::character varying(256) AS carrier_policy_number,
    'raw_lmi_cau_pd'::character varying(256) AS source_table,
    a.uploaded_at
   FROM p_and_c.raw_lmi_cau_pd a;
GO

/*** PERMISSIONS ***/
ALTER VIEW p_and_c.normal_lmi_cau_pd OWNER TO mj_admin;
GO

Grant select on table p_and_c.normal_lmi_cau_pd to rl_pc_data_analyst;
GO

Grant select on table p_and_c.normal_lmi_cau_pd to rl_pc_rpa_developer;
GO

Grant select on table p_and_c.normal_lmi_cau_pd to rpauser;
GO
GRANT SELECT ON p_and_c.normal_lmi_cau_pd to rl_p_and_c_r;
GO

GRANT INSERT ON p_and_c.normal_lmi_cau_pd to rl_p_and_c_a;
GO

GRANT UPDATE ON p_and_c.normal_lmi_cau_pd to rl_p_and_c_w;
GO

GRANT DELETE ON p_and_c.normal_lmi_cau_pd to rl_p_and_c_d;
GO