CREATE OR REPLACE VIEW p_and_c.all_wc_ldf
AS SELECT a.state,
    a."12_months" AS "6_months",
    ...
    a."108_months"
   FROM p_and_c.wc_ldf a
  WHERE a.uploaded_at = (( SELECT max(wc_ldf.uploaded_at) AS max
           FROM p_and_c.wc_ldf
          WHERE wc_ldf.state = a.state));
GO

/*** PERMISSIONS ***/
ALTER VIEW p_and_c.all_wc_ldf OWNER TO mj_admin;
GO

Grant select on table p_and_c.all_wc_ldf to rl_pc_data_analyst;
GO

Grant select on table p_and_c.all_wc_ldf to rl_pc_rpa_developer;
GO

Grant select on table p_and_c.all_wc_ldf to rpauser;
GO
GRANT SELECT ON p_and_c.all_wc_ldf to rl_p_and_c_r;
GO

GRANT INSERT ON p_and_c.all_wc_ldf to rl_p_and_c_a;
GO

GRANT UPDATE ON p_and_c.all_wc_ldf to rl_p_and_c_w;
GO

GRANT DELETE ON p_and_c.all_wc_ldf to rl_p_and_c_d;
GO