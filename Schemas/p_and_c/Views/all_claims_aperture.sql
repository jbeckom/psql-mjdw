CREATE OR REPLACE VIEW p_and_c.all_claims_aperture
AS SELECT g.claim_number,
    ...
    g.subrogation,
    g.reimbursed,
    COALESCE(v.claim_status, g.claim_status) AS claim_status,
    g.description_text,
    ...
    g.class_code,
    upper(g.claimant_name::text)::character varying(256) AS claimant_name,
    upper(g.claimant_first::text)::character varying(256) AS claimant_first,
    upper(g.claimant_middle::text)::character varying(256) AS claimant_middle,
    upper(g.claimant_last::text)::character varying(256) AS claimant_last,
    upper(g.driver_first::text)::character varying(256) AS driver_first,
    upper(g.driver_middle::text)::character varying(256) AS driver_middle,
    upper(g.driver_last::text)::character varying(256) AS driver_last,
    g.loss_date,
    g.loss_time,
    g.report_date,
    g.closed_date,
   ...
   FROM ( SELECT e.claim_number,
           ...
           FROM ( SELECT a.claim_number,
                    ...
                    COALESCE(d.claimant_first,a.claimant_first) AS claimant_first,
                    COALESCE(d.claimant_middle,a.claimant_middle) AS claimant_middle,
                    COALESCE(d.claimant_last,a.claimant_last) AS claimant_last,
                    COALESCE(d.driver_first,a.driver_first) AS driver_first,
                    COALESCE(d.driver_middle,a.driver_middle) AS driver_middle,
                    COALESCE(d.driver_last,a.driver_last) AS driver_last,
                    a.loss_date,
                    a.loss_time,
                    a.report_date,
                    a.closed_date,
                   ...
                   FROM p_and_c.all_claims_monthly a
                     ...
                     LEFT JOIN p_and_c.mapping_by_claim_number d ON c.client_id::text = d.client_id::text AND a.claim_number::text = d.claim_number::text) e
...
/*** PERMISSIONS ***/
ALTER VIEW p_and_c.all_claims_aperture OWNER TO mj_admin;
GO

Grant select on table p_and_c.all_claims_aperture to rl_pc_data_analyst;
GO

Grant select on table p_and_c.all_claims_aperture to rl_pc_rpa_developer;
GO

Grant select on table p_and_c.all_claims_aperture to rpauser;
GO
GRANT SELECT ON p_and_c.all_claims_aperture to rl_p_and_c_r;
GO

GRANT INSERT ON p_and_c.all_claims_aperture to rl_p_and_c_a;
GO

GRANT UPDATE ON p_and_c.all_claims_aperture to rl_p_and_c_w;
GO

GRANT DELETE ON p_and_c.all_claims_aperture to rl_p_and_c_d;
GO